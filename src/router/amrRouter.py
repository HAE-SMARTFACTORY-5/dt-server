...
async def updateAmr(amrId: int, request: amrDto.AmrRequest) -> str:
    ...
    websocketService.sendBroadcastWithActionType(SocketGroup.AMR, SocketActionType.AMR, amrResponse)
    websocketService.sendToUserInRoomWithActionType(SocketGroup.AMR, amrId, amrResponse)
    return "OK"
